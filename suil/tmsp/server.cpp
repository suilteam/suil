...
namespace suil::tmsp {
    ...
    int AbciConn::receiveLen(const char *dbg) {
        ...
        if (readProtobufSize(this, dbg, sock, size) != 0) {
            // receiving lsz failed
            idebug("%s - receiving message size failed: %s", dbg, errno_s);
            return -errno;
        }
    ...
    bool AbciConn::receiveMsg(OBuffer &rxb, size_t len, const char *dbg) {
        char *data = rxb.data();
    ...
    void AbciConn::start() {
        String addr(utils::catstr(ipstr(sock.addr()), ":", sock.port()));
        // enter receive loop
        OBuffer rxb;
        while (sock.isopen()) {
            // receive the expected message length
            int msglen = receiveLen(addr());
            if (msglen <= 0) {
    ...
    void AbciConn::handle(const types::Request &req, types::Response &resp) {
        switch (req.value_case()) {
            ...
            case types::Request::ValueCase::kDeliverTx: {
                ...
                Data tx{(uint8_t *) rqdtx.tx().data(), rqdtx.tx().size(), false};
            ...
            case types::Request::ValueCase::kCheckTx: {
                ...
                Data tx{(uint8_t *) rqctx.tx().data(), rqctx.tx().size(), false};
